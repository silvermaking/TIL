...
* 순서가 없는 목록입니다.
  * tab을 누르면, 하위 목록을 작성가능
* shift + tab하시면 상위 목록을 작성가능
...
코드블록은 backtick(`)  세 개로 구성되며, 특정 언어를 같이 작성가능
...
ex) `random`  모듈을 활용하는 것을 배웠다. `print` 함수는 출력할 때 사용된다. 
